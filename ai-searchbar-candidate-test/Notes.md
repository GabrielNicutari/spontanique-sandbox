...
- Added logic to detect if the query contains only time-based words (e.g., "today", "tonight", "weekend"). In such cases, we should not penalize events for lacking content matches, as the user's intent is purely temporal. This should help improve the relevance of results for time-only queries.
   - At the same time, for a query like "jazz tonight", we still want to prioritize events that match the content ("jazz") while also considering the time aspect ("tonight"). This dual consideration should help surface the most relevant events for such mixed queries.
   - This won't be perfect still. It would need a lot of hard-coding/data to cover all possible time-based words, and even then, there will be edge cases. A more robust solution will involve actual NLP techniques to understand the intent behind the query better. This is a basic heuristic to improve the current implementation.